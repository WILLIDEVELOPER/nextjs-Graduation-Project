  ...
  const updateUser = async (id) => {
    if (id) {
      try {
        ...
        setMessageUpdate("Usuario Actualizado con exito");
        console.log(respuesta);
      } catch (error) {
        setMessageUpdate(
          "Algun parametro o algun error de red no permitio aactualizar"
        );
        console.log(error);
      }
    }
        ...
        <div className="w-full  flex items-center justify-center">
          ...
          <div className=" flex flex-col gap-[1.5rem]">
            ...
            <h1 className="text-center text-xl">
              {getAdmin ? getAdmin?.username : getUsuario?.username}
            </h1>
          </div>
        </div>
        <div>
